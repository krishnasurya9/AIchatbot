...
async def retrieve_context_multi_source(
    ...
    try:
        ...
        try:
            # Escape regex special characters to avoid MongoDB errors on complex queries
            safe_query = re.escape(query)
            fallback_filter = {"content": {"$regex": safe_query, "$options": "i"}}
            ...
            logger.error(f"RAG fallback failed: {fallback_err}", exc_info=True)
            results = []
